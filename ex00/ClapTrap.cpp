/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ClapTrap.cpp                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: fmaurer <fmaurer42@posteo.de>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/20 20:44:36 by fmaurer           #+#    #+#             */
/*   Updated: 2025/08/03 17:57:27 by fmaurer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ClapTrap.hpp"
#include "utils.hpp"

#include <iostream>

ClapTrap::ClapTrap(): _name("noname"), _hp(10), _nrg(10), _dmg(0)
{
  dbg_msg("ClapTrap " + this->_name, "Default Constructor called.");
  this->_msgPrefix = get_prefix(this->_name);
}

ClapTrap::ClapTrap(const std::string& name):
  _name(name), _hp(10), _nrg(10), _dmg(0)
{
  dbg_msg("ClapTrap " + this->_name, "Default-Name Constructor called.");
  this->_msgPrefix = get_prefix(this->_name);
}

ClapTrap::ClapTrap(const ClapTrap& other)
{
  dbg_msg("ClapTrap " + this->_name, "Copy-Constructor called.");
  if (this != &other)
  {
    this->_name      = other._name;
    this->_hp        = other._hp;
    this->_nrg       = other._nrg;
    this->_dmg       = other._dmg;
    this->_msgPrefix = get_prefix(this->_name);
  }
}

ClapTrap& ClapTrap::operator=(const ClapTrap& other)
{
  dbg_msg("ClapTrap " + this->_name, "Assignment-Constructor called.");
  if (this != &other)
  {
    this->_name      = other._name;
    this->_hp        = other._hp;
    this->_nrg       = other._nrg;
    this->_dmg       = other._dmg;
    this->_msgPrefix = get_prefix(this->_name);
  }
  return (*this);
}

ClapTrap::~ClapTrap()
{
  dbg_msg("ClapTrap " + this->_name, "Destructor called.");
}

void ClapTrap::beRepaired(const unsigned int& amount)
{
  if (this->_hp == -1)
  {
    std::cout << this->_msgPrefix << "already dead!" << std::endl;
    return;
  }
  if (this->_nrg == 0)
  {
    std::cout << this->_msgPrefix << "has 0 energy. No more repair possible!"
              << std::endl;
    return;
  }
  this->_hp += amount;
  this->_nrg--;
  std::cout << this->_msgPrefix << "is being repaired by " << amount
            << " hp. Remaining HP: " << this->_hp
            << ", remaining energy: " << this->_nrg << std::endl;
}

void ClapTrap::takeDamage(const unsigned int& amount)
{
  if (this->_hp == -1)
  {
    std::cout << this->_msgPrefix << "takes " << amount
              << " hp damage, was already dead before, now is even deader!"
              << std::endl;
    return;
  }
  if (_hp > amount)
  {
    this->_hp -= amount;
    std::cout << this->_msgPrefix << "takes " << amount
              << " hp of damage. Remaining HP: " << this->_hp << std::endl;
  } else if (this->_hp == amount)
  {
    std::cout << this->_msgPrefix << "takes " << amount
              << " hp of damage. Remaining HP: 0. One more strike till death! "
              << std::endl;
    this->_hp = 0;

  } else
  {
    std::cout << this->_msgPrefix << "takes " << amount
              << " hp of damage. HP < 0 -> Dead! " << std::endl;
    this->_hp = -1;
  }
}

void ClapTrap::attack(const std::string& target)
{
  if (this->_hp == -1)
  {
    std::cout << this->_msgPrefix << "already dead!" << std::endl;
    return;
  }
  if (this->_nrg == 0)
  {
    std::cout << this->_msgPrefix << "has 0 energy. No more attacks possible!"
              << std::endl;
    return;
  }
  this->_nrg--;
  std::cout << this->_msgPrefix << "attacks " << target << ", causing "
            << this->_dmg << " points of damage! Energy: " << this->_nrg
            << std::endl;
}
